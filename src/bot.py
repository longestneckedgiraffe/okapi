"""
"Okapi" is a Discord bot that uses Mistral's API to provide intelligent,
conversational interactions in your Discord server.

Responses generated by Okapi do not represent the views of the creator
(Ridhwan Zaman) or Mistral. The bot is provided as-is without warranty of
any kind, and may produce inaccurate or incorrect information.

Okapi is not immune to prompt injections and may respond to malicious prompts
in unintended ways (learn more: https://www.ibm.com/think/topics/prompt-injection).
Conversation history is stored locally and temporarily to maintain context within
channels. Conversations are pruned for relevance and can be cleared at any time
using the /amnesia command. When a server key is configured, conversation data
is encrypted at rest using AEAD (AES-GCM). Okapi does not store or transmit
sensitive information beyond what is needed to fulfill your request.

THIS BOT IS NOT A SUBSTITUTE FOR PROFESSIONAL ADVICE OF ANY KIND, medical, legal,
financial, or otherwise. Always verify important information from authoritative
sources and do not rely on Okapi for critical decisions. The creator assumes no
liability for actions taken based on the bot's responses.

By using Okapi, you agree to Mistral's Terms of Service
(https://mistral.ai/terms) and acknowledge the data handling practices described above.
